...
export default function HeroCGP() {
  const { t } = useTranslation()
  const signals = t('heroCgp.signals', { returnObjects: true }) as Signal[]
  const gaps = t('heroCgp.gaps', { returnObjects: true }) as Gap[]
  const protocolMeta = t('heroCgp.protocolMeta', { returnObjects: true }) as ProtocolMeta[]
  ...
  return (
    <section
      ...
      aria-labelledby="hero-cgp-title"
    >
      {/* Copy */}
      <div className="relative z-20 max-w-[720px]">
        ...
        <div className="flex flex-wrap gap-3 mt-8">
          <Button as="a" href="/sample?type=cgp" variant="primary">
            {t('heroCgp.ctaPrimary')}
          </Button>
          <Button as="a" href="#inquiry" variant="secondary">
            {t('heroCgp.ctaSecondary')}
          </Button>
        </div>
        ...
        <div className="absolute right-[18px] top-0 z-10 w-[min(100%,390px)] min-h-[500px] border border-ink/25 bg-gradient-to-b from-[#fffdf6] to-[#ede6d9] shadow-[0_24px_70px_rgba(28,31,27,0.13)] p-9">
          ...
          <div className="border border-ink/12 divide-y divide-ink/10">
            <div className="grid grid-cols-[1fr_auto] gap-3 px-3 py-1.5 bg-ink/5">
              <span className="font-mono text-[10px] uppercase tracking-widest text-ink/50">{t('heroCgp.artifactCondition')}</span>
              <span className="font-mono text-[10px] uppercase tracking-widest text-ink/50">{t('heroCgp.artifactSeverity')}</span>
            </div>
            {signals.map((s) => (
          ...
          <p className="mt-5 font-mono text-[10px] uppercase tracking-widest text-ink/40 mb-2">
            {t('heroCgp.artifactEvidenceGaps')}
          </p>
          <div className="border border-ink/12 divide-y divide-ink/10">
            {gaps.map((g) => (
