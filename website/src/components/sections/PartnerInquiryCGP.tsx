...
// CGP variant of PartnerInquiry — patrimoine sample panel + CGP qualification form, sends to contact@symi.io.
export default function PartnerInquiryCGP() {
  const { t } = useTranslation()
  const [form, setForm] = useState<FormState>({ context: '', jurisdiction: '', client: '' })
  const [status, setStatus] = useState('')
  const samplePath = '/samples/FR-PATRIMOINE-CGP_SAMPLE.md'
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.context || !form.jurisdiction || !form.client) {
      setStatus(t('inquiryCgp.validationError'))
      return
    }
    const subject = encodeURIComponent('Demande partenaire CGP — Exposure Intelligence')
    const body = encodeURIComponent(
      [
        'Demande partenaire CGP',
        '',
        `Profil client : ${form.context}`,
        `Zone : ${form.jurisdiction}`,
        `Client test : ${form.client}`,
      ].join('\n')
    )
    setStatus(t('inquiryCgp.successMessage'))
    window.location.href = `mailto:contact@symi.io?subject=${subject}&body=${body}`
  }
  ...
  return (
    ...
    >
      ...
      <div className="lg:sticky lg:top-[92px] p-[clamp(28px,4vw,44px)] border border-ink/12 bg-ink text-paper shadow-[0_18px_50px_rgba(32,35,31,0.1)]">
        <p className="text-2xs font-bold uppercase tracking-[0.14em] text-copper mb-2">
          {t('inquiryCgp.sampleEyebrow')}
        </p>
        <p className="font-mono text-[11px] text-paper/45 tracking-[0.08em] mb-3.5">
          {t('inquiryCgp.sampleKicker')}
        </p>
        <h2 id="sample-title" className="text-[clamp(28px,3.5vw,44px)]">
          {t('inquiryCgp.sampleHeading')}
        </h2>
        <p className="mt-4 text-[17px] text-paper/70 leading-[1.55]">
          {t('inquiryCgp.sampleBody')}
        </p>
        <div className="mt-7 flex flex-wrap gap-3">
          <Button as="a" href="/sample?type=cgp" variant="light">
            {t('inquiryCgp.sampleCta')}
          </Button>
          <a
            href={samplePath}
            download
      ...
      >
        <p className="text-2xs font-bold uppercase tracking-[0.14em] text-copper mb-3">
          {t('inquiryCgp.formEyebrow')}
        </p>
        <h2 className="text-[clamp(26px,3vw,38px)]">{t('inquiryCgp.formHeading')}</h2>
        <p className="mt-3 text-[15px] text-ink-soft leading-[1.5]">{t('inquiryCgp.formSubhead')}</p>

        {/* Q1 */}
        <fieldset className="mt-7 border-0 p-0">
          <legend className="font-serif text-[19px] font-bold text-ink mb-1">
            {t('inquiryCgp.q1legend')}
          </legend>
          {(['q1a', 'q1b', 'q1c', 'q1d'] as const).map((key) => {
            const val = t(`inquiryCgp.${key}`)
            return (
        ...
        <fieldset className="mt-7 border-0 p-0">
          <legend className="font-serif text-[19px] font-bold text-ink mb-1">
            {t('inquiryCgp.q2legend')}
          </legend>
          <div className="grid grid-cols-2 gap-2.5 mt-2.5">
            {(['q2a', 'q2b', 'q2c', 'q2d'] as const).map((key) => {
              const val = t(`inquiryCgp.${key}`)
              return (
